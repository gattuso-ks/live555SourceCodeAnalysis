...
int GenericMediaServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort) {
  ...
  do {
    ...
    ourSocket = setupStreamSocket(env, ourPort, True/*设置非阻塞*/, True/*设置内置心跳*/);//返回一个设置好属性的socket
    if (ourSocket < 0) break;
    
    // Make sure we have a big send buffer:
    if (!increaseSendBufferTo(env, ourSocket, 50*1024)) break;//尝试将sendbuffer设置成50×1024那么大
    
    // Allow multiple simultaneous connections:
    if (listen(ourSocket, LISTEN_BACKLOG_SIZE) < 0) {//监听 半连接队列长度为宏定义 此处为20
      env.setResultErrMsg("listen() failed: ");
      break;
    }
    
    if (ourPort.num() == 0) {//如果ourPort中的端口号为0
