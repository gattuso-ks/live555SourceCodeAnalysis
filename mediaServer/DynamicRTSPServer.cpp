...
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  int ourSocket = setUpOurSocket(env, ourPort);//生成一个符合要求的socket
  if (ourSocket == -1) return NULL;

  return new DynamicRTSPServer(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds);//调用构造
}

DynamicRTSPServer::DynamicRTSPServer(UsageEnvironment& env, int ourSocket,
				     Port ourPort,
				     UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
  : RTSPServerSupportingHTTPStreaming(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds)/*初始化列表*/ {
}
