...
int setupStreamSocket(UsageEnvironment& env,
  ...
  int newSocket = createSocket(SOCK_STREAM);//创建一个SOCK_STREAM的socket
  if (newSocket < 0) {
    socketErr(env, "unable to create stream socket: ");
    return newSocket;
  }

  int reuseFlag = groupsockPriv(env)->reuseFlag;//获得env实例中的reuseflag的值
  reclaimGroupsockPriv(env);
  //设置socket选项 reuseflag指向一个新选项的地址 
  //SO_REUSEADDR 可以绑定一个已使用的地址
  //也就是可以使用在TIME_WAIT状态的地址
  //也就是通过reuseflag这个参数标记这个socket是否开启这个功能
  if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEADDR,
		 (const char*)&reuseFlag, sizeof reuseFlag) < 0) {
    socketErr(env, "setsockopt(SO_REUSEADDR) error: ");
    closeSocket(newSocket);
...
#else
#ifdef SO_REUSEPORT
  //REUSEPORT可以让多个socketfd绑定同一个ip和端口，实现内核层面的负载均衡
  if (setsockopt(newSocket, SOL_SOCKET, SO_REUSEPORT,
		 (const char*)&reuseFlag, sizeof reuseFlag) < 0) {
    socketErr(env, "setsockopt(SO_REUSEPORT) error: ");
    closeSocket(newSocket);
  ...
  if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
#endif
    MAKE_SOCKADDR_IN(name, ReceivingInterfaceAddr, port.num());//生成一个以name为名字的struct socketaddr_in ip地址为Rec…… 端口号为port.num()
    if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
      char tmpBuffer[100];
      sprintf(tmpBuffer, "bind() error (port number: %d): ",
	      ntohs(port.num()));
  ...
  if (makeNonBlocking) {
    if (!makeSocketNonBlocking(newSocket)) {//设置非阻塞
  ...
  if (setKeepAlive) {
    if (!setSocketKeepAlive(newSocket)) {//设置TCP内置心跳机制
...
static unsigned getBufferSize(UsageEnvironment& env, int bufOptName,
  ...
  if (getsockopt(socket, SOL_SOCKET, bufOptName,//获取当前的sendbuffer大小
...
static unsigned increaseBufferTo(UsageEnvironment& env, int bufOptName,
  ...
  unsigned curSize = getBufferSize(env, bufOptName, socket);//获取当前sendbuffer大小
  ...
  //尝试将sendbuffer大小设置为传进来的参数那么大，如果失败就尝试(reqsize+cursize)/2 并返回设置的大小
    SOCKLEN_T sizeSize = sizeof requestedSize;
...
static Boolean getSourcePort0(int socket, portNumBits& resultPortNum/*host order*/) {
  ...
  if (getsockname(socket, (struct sockaddr*)&test, &len) < 0) return False;//获取socket绑定了的socketaddr_in

  resultPortNum = ntohs(test.sin_port);//将获得的端口号传出
  return True;
}
...
Boolean getSourcePort(UsageEnvironment& env, int socket, Port& port) {
  ...
  if (!getSourcePort0(socket, portNum) || portNum == 0) {
    ...
    MAKE_SOCKADDR_IN(name, INADDR_ANY, 0);//重新生成一个name为名的socketaddr_in 绑定INADDR_ANY的ip地址
    bind(socket, (struct sockaddr*)&name, sizeof name);//重新绑定
